...
const navigate = useNavigate();
  ...
  const getCompanyFromLocalStorage = () => {
    const getResult = localStorage.getItem('result001');
    if (getResult) {
      const companyData = JSON.parse(getResult);
      setCompanyName(companyData.companyName);
      setEmail(companyData.email);
      setMobile(companyData.mobile);
      setStatus(companyData.status);
      setWebsite(companyData.website);
      setOther(companyData.other);
      setAddress01(companyData.address01);
      setAddress02(companyData.address02);
      setLandmark(companyData.landmark);
      setPin(companyData.pin);
      setGstno(companyData.gstno);
      setOtherTax01(companyData.othertax01);
      setOtherTax02(companyData.othertax02);
      setSelectedCountry(companyData.selectedCountry);
      setSelectedState(companyData.selectedState);
      setSelectedCity(companyData.selectedCity);
      // Set other state variables here as needed
    }
  };
  ...
  const storedId = localStorage.getItem("result001");

const   myId = JSON.parse(storedId.replace(/^"(.*)"$/, '$1'));
...
// alert(myId._id)
  const updatecompany = () => {
    ...
    fetch(`http://localhost:3000/dev/edit_company/${myId}`, requestOptions)
      .then(response => response.json())
      .then(result => {
        console.log(result)
        console.log(result)
          ...
          <div className="container-fluid">
            <div className="row mb-2">
              <div className="col-sm-6">
                <h1>{languageData.companyconfiguration}</h1>
              </div>
              <div className="col-sm-6">
...
export default CompanyConfigure